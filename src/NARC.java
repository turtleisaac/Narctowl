...
public class NARC
{
    ...
    {
        NARC narc= new NARC(); //creates new NARC object
    ...
    private String extractPath= path + "extracted" + File.separator; //creates a new String field containing path, the directory "extracted", and File.separator (/ on Unix systems, \ on Windows)
    ...
    public NARC() throws IOException
    {
    ...
    public void unpack(String narc) throws Exception
    {
        ...
        if(!new File(path + "extracted" + File.separator + noExtension).exists()) //checks if "extracted" directory in user.dir does not exist
        {
            if (!new File(path + "extracted" + File.separator + noExtension).mkdirs()) //if it does not exist, creates "extracted" directory and directory matching name of parameter narc if able to, otherwise see RuntimeException
            {
        ...
        else //if a folder matching the name of the input narc already exists within "extracted"
        {
            while(true) //this makes sure that the program will keep asking if the user inputs an invalid response
            {
                ...
                {
                    if(!clearDirectory(new File(path + "extracted" + File.separator + noExtension))) //clears specified directory if able to, otherwise see RuntimeException
                    {
                        throw new RuntimeException("Could not delete contents of existing " + noExtension + " directory. Check write permissions");
                    }
                    if(!new File(path + "extracted" + File.separator + noExtension).delete()) //deletes existing specified directory if able to, otherwise see RuntimeException
                    {
                        throw new RuntimeException("Could not delete existing " + noExtension + " directory. Check write permissions");
                    }
                    if(!new File(path + "extracted" + File.separator + noExtension).mkdir()) //creates new folder matching the name of the input narc if able to, otherwise see RuntimeException
                    {
    ...
    {
        if(new File(path + name + ".narc").exists()) //if a narc matching parameter name exists
        {
...
//        }
        ...
        BinaryWriter narcWriter= new BinaryWriter(path + name + ".narc"); //creates new file to output to in user directory with parameter "name" + .narc
        ...
        System.out.println("Process completed. Output file can be found at: " + path + name + ".narc");
    ...
    {
        Arrays.sort(arr, Comparator.comparingInt(NARC::fileToInt));
    }
